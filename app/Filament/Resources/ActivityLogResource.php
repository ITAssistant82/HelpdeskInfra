...
class ActivityLogResource extends Resource
{
    ...
    {
        return $table->defaultSort('created_at', 'desc')->columns([Tables\Columns\TextColumn::make('log_name')->label('Log')->badge()->colors(['info' => 'default']), Tables\Columns\TextColumn::make('description')->label('Aktivitas')->searchable()->limit(50), Tables\Columns\TextColumn::make('subject_type')->label('Model')->formatStateUsing(fn ($state) => class_basename($state))->badge()->color('gray'), Tables\Columns\TextColumn::make('causer_id')->label('User')->formatStateUsing(fn ($record) => $record->causer?->name ?? $record->causer?->email ?? '-'), Tables\Columns\TextColumn::make('created_at')->label('Waktu')->dateTime('d M Y H:i:s')->sortable(), ])->filters([Tables\Filters\SelectFilter::make('log_name')->label('Jenis Log')->options(fn () => \Spatie\Activitylog\Models\Activity::distinct()->pluck('log_name', 'log_name')->toArray()), \Filament\Tables\Filters\Filter::make('causer')->form([\Filament\Forms\Components\Select::make('causer_id')->label('User')->searchable()->options(\App\Models\User::all()->pluck('name', 'id')->toArray()), ])->query(function ($query, array $data) {
            return $query->when($data ['causer_id'], fn ($q) => $q->where('causer_id', $data ['causer_id'])->where('causer_type', \App\Models\User::class));
