...
class EmployeeAssetResource extends Resource
{
    ...
    $lastCode = \Illuminate\Support\Facades\DB::table('employee_assets')->orderBy('id', 'desc')->value('asset_code');
    if ($lastCode) {
    ...
    {
        return $table->columns([Tables\Columns\TextColumn::make('asset_code')->searchable()->sortable(), Tables\Columns\TextColumn::make('employee.nik')->label('NIK')->searchable()->sortable(), Tables\Columns\TextColumn::make('employee.full_name')->label('Employee')->searchable()->sortable(), Tables\Columns\TextColumn::make('asset_type')->badge()->color(fn ($state) => match ($state) { 'Laptop' => 'warning', 'PC' => 'info', 'Pribadi' => 'success', 'Smartphone' => 'danger', default => 'gray' })->sortable(), Tables\Columns\TextColumn::make('brand')->searchable()->sortable(), Tables\Columns\TextColumn::make('model')->searchable()->sortable(), Tables\Columns\TextColumn::make('location')->label('Lokasi')->badge()->color(fn ($state) => match ($state) { 'BSD' => 'info', 'Cilandak' => 'warning', 'Mobile' => 'gray', default => 'gray' })->sortable(), Tables\Columns\TextColumn::make('os')->label('OS')->searchable()->sortable(), Tables\Columns\TextColumn::make('processor')->label('Processor')->searchable()->sortable(), Tables\Columns\TextColumn::make('memory_gb')->label('RAM (GB)')->sortable()->numeric(decimalPlaces: 1), Tables\Columns\TextColumn::make('hard_drive_gb')->label('Storage (GB)')->sortable()->numeric(decimalPlaces: 0), Tables\Columns\TextColumn::make('condition')->badge()->color(fn ($state) => match ($state) { 'Baik' => 'success', 'Perlu Perawatan' => 'warning', 'Rusak' => 'danger', default => 'gray' })->sortable(), Tables\Columns\TextColumn::make('tahun_pembelian')->label('Tahun Beli')->sortable(), Tables\Columns\TextColumn::make('usia')->label('Usia Perangkat')->badge()->color(fn ($record) => $record->usia && (int) $record->tahun_pembelian && (now()->year - (int) $record->tahun_pembelian) >= 10 ? 'danger' : ((now()->year - (int) $record->tahun_pembelian) >= 5 ? 'warning' : 'success'))->sortable(query: fn ($query, $direction) => $query->orderBy('tahun_pembelian', $direction)), Tables\Columns\TextColumn::make('assigned_at')->label('Diberikan')->date()->sortable(), ])->filters([Tables\Filters\SelectFilter::make('asset_type')->label('Jenis Perangkat')->options(['Laptop' => 'Laptop', 'PC' => 'PC', 'Smartphone' => 'Smartphone', 'Pribadi' => 'Pribadi']), Tables\Filters\SelectFilter::make('location')->label('Lokasi')->options(['BSD' => 'BSD', 'Cilandak' => 'Cilandak', 'Mobile' => 'Mobile']), Tables\Filters\SelectFilter::make('condition')->label('Kondisi')->options(['Baik' => 'Baik', 'Perlu Perawatan' => 'Perlu Perawatan', 'Rusak' => 'Rusak', ]), ])->actions([Actions\Action::make('preview_barcode')->label('Preview Barcode')->icon('heroicon-o-qr-code')->color('info')->url(fn (EmployeeAsset $record): string => route('asset.barcode.preview', $record))->openUrlInNewTab(), Actions\Action::make('print_sticker')->label('Cetak Stiker')->icon('heroicon-o-tag')->color('danger')->url(fn (EmployeeAsset $record): string => route('asset.barcode.sticker', $record))->openUrlInNewTab(), Actions\EditAction::make(), ])->bulkActions([Actions\BulkActionGroup::make([Actions\DeleteBulkAction::make(), ]), ]);
    }
