...
class AssetsRelationManager extends RelationManager
{
    ...
    {
        return $table->columns([Tables\Columns\TextColumn::make('asset_code')->searchable()->sortable(), Tables\Columns\TextColumn::make('asset_type')->badge()->color(fn ($state) => match ($state) { 'Laptop' => 'warning', 'PC' => 'info', default => 'gray' }), Tables\Columns\TextColumn::make('brand')->searchable(), Tables\Columns\TextColumn::make('model')->searchable(), Tables\Columns\TextColumn::make('os')->label('OS')->searchable(), Tables\Columns\TextColumn::make('processor')->label('Processor')->searchable(), Tables\Columns\TextColumn::make('memory_gb')->label('RAM (GB)')->numeric(decimalPlaces: 1), Tables\Columns\TextColumn::make('condition')->badge()->color(fn ($state) => match ($state) { 'Baik' => 'success', 'Perlu Perawatan' => 'warning', 'Rusak' => 'danger', default => 'gray' }), Tables\Columns\TextColumn::make('assigned_at')->label('Diberikan')->date(), ])->headerActions([
