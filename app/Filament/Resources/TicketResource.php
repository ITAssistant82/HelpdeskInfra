...
class TicketResource extends Resource
{
    ...
    {
        return [Schemas\Components\Section::make('Informasi Tiket')->columns(2)->schema([Forms\Components\Select::make('type')->label('Tipe')->options(['Incident' => 'Incident', 'Service Request' => 'Service Request'])->required()->live(), Forms\Components\Select::make('category_id')->label('Kategori')->options(fn ($get) => TicketCategory::query()->where('is_active', true)->when($get('type'), fn ($q, $type) => $q->where('type', $type))->get()->mapWithKeys(fn ($cat) => [$cat->id => " { $cat->main_category}
 - { $cat->sub_category}
 "]))->required()->live(), Forms\Components\TextInput::make('title')->label('Judul')->required()->columnSpanFull(), Forms\Components\Textarea::make('description')->label('Deskripsi')->required()->columnSpanFull()->rows(3), ]), Schemas\Components\Section::make('Detail')->columns(2)->schema([Forms\Components\Select::make('location')->label('Lokasi')->options(['BSD' => 'BSD', 'Cilandak' => 'Cilandak', 'Remote' => 'Remote', 'Cloud' => 'Cloud'])->required(), Forms\Components\TextInput::make('requester_unit')->label('Unit / Departemen'), Forms\Components\Placeholder::make('assignee_name')->label('Teknisi yang Menangani')->content(fn ($record) => $record?->assignee?->name)->visible(fn ($record) => filled($record?->assigned_to))->columnSpanFull()]), Schemas\Components\Section::make('Lampiran')->schema([Forms\Components\Placeholder::make('attachments_existing')->label('')->content(fn ($record) => new HtmlString(view('components.ticket-attachments', ['attachments' => $record?->attachments ?? collect()])->render()))->visible(fn ($operation, $record) => ($operation === 'edit' || $operation === 'view') && $record), Forms\Components\FileUpload::make('new_attachments')->label('Upload File')->disk('public')->directory('ticket-attachments')->preserveFilenames()->multiple()->visible(fn ($operation) => $operation !== 'view')->columnSpanFull()]), ];
    ...
    {
        return [Schemas\Components\Section::make('Informasi Tiket')->columns(2)->schema([Forms\Components\TextInput::make('ticket_number')->label('Nomor Tiket')->disabled()->dehydrated(false)->visible(fn ($operation) => $operation === 'edit'), Forms\Components\Select::make('type')->label('Tipe')->options(['Incident' => 'Incident', 'Service Request' => 'Service Request'])->required()->live(), Forms\Components\Select::make('category_id')->label('Kategori')->options(fn ($get) => TicketCategory::query()->where('is_active', true)->when($get('type'), fn ($q, $type) => $q->where('type', $type))->get()->mapWithKeys(fn ($cat) => [$cat->id => " { $cat->main_category}
 - { $cat->sub_category}
