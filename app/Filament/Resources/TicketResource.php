...
class TicketResource extends Resource
{
    ...
    public static function canManageImportsAndExports(): bool
    {
 ...
 "]))->required()->live()->afterStateUpdated(fn ($state, $set) => static::updateAssignedGroup($state, $set)), Forms\Components\TextInput::make('title')->label('Judul')->required()->columnSpanFull(), Forms\Components\Textarea::make('description')->label('Deskripsi')->required()->columnSpanFull()->rows(3), ]),         Schemas\Components\Section::make('Pemohon')->columns(2)->schema([Forms\Components\Select::make('requester_id')->label('Pemohon')->relationship('requester', 'name')->default(Auth::id())->disabled()->required(), Forms\Components\Select::make('location')->label('Lokasi')->options(['BSD' => 'BSD', 'Cilandak' => 'Cilandak', 'Remote' => 'Remote', 'Cloud' => 'Cloud'])->required(), Forms\Components\TextInput::make('requester_unit')->label('Unit / Departemen')]), Schemas\Components\Section::make('Prioritas & Penugasan')->columns(2)->schema([Forms\Components\Select::make('urgency')->label('Urgensi')->options(['Low' => 'Low', 'Medium' => 'Medium', 'High' => 'High', 'Critical' => 'Critical'])->required(), Forms\Components\Select::make('assigned_to')->label('Ditugaskan Ke')->options(fn ($record, $get) => static::getAssignableUsers(
    ...
    public static function table(Table $table): Table
    {
        ...
        return $table->poll('30s')->columns([Tables\Columns\TextColumn::make('ticket_number')->searchable()->sortable()->html()->formatStateUsing(fn ($record) => $record?->isOutsideWorkingHours() ? $record->ticket_number.' <span class="text-xs font-medium text-warning-600 bg-warning-100 rounded px-1 py-0.5">Out of Hours</span>' : $record->ticket_number), Tables\Columns\TextColumn::make('type')->badge()->color(fn ($state) => TicketType::colorFor($state))->searchable(), Tables\Columns\TextColumn::make('title')->searchable()->limit(40), Tables\Columns\TextColumn::make('category.main_category')->searchable(), Tables\Columns\TextColumn::make('assignee.name')->label('Teknisi')->placeholder('Belum ditugaskan')->visible(! $isStaff), Tables\Columns\TextColumn::make('priority')->state(fn (Ticket $record): ?string => $record->urgency ? Ticket::calculatePriority($record->impact ?? 'Medium', $record->urgency) : null)->placeholder('-')->badge()->color(fn ($state) => match ($state) {
        ...
        }), Tables\Columns\TextColumn::make('outside_working_hours')->label('Jam Kerja')->badge()->state(fn ($record) => $record?->isOutsideWorkingHours() ? 'Di Luar Jam Kerja' : '')->color(fn ($state) => $state === 'Di Luar Jam Kerja' ? 'warning' : '')->formatStateUsing(fn ($state) => $state)->visible(fn ($record) => $record ? $record->isOutsideWorkingHours() : false), Tables\Columns\TextColumn::make('helpers_summon')->label('Summon')->badge()->state(fn ($record) => $record?->helpers?->pluck('name')->toArray() ?? [])->color('info')->visible(fn ($record) => $record?->helpers?->isNotEmpty() ?? false), Tables\Columns\TextColumn::make('created_at')->dateTime('d/m/Y H:i')->sortable(), Tables\Columns\TextColumn::make('sla_deadline')->label('SLA Deadline')->dateTime('d/m/Y H:i')->sortable()->visible($isStaff), Tables\Columns\TextColumn::make('sla_status')->label('SLA')->badge()->state(fn ($record) => $record->slastatus())->color(fn ($state) => match ($state) {
        ...
        })->visible($isStaff),         ])->filters([Tables\Filters\SelectFilter::make('status')->options(['New' => 'New', 'Assigned' => 'Assigned', 'In Progress' => 'In Progress', 'Pending User' => 'Pending User', 'Pending Approval' => 'Pending Approval', 'Pending Vendor' => 'Pending Vendor', 'Pending Procurement' => 'Pending Procurement', 'Escalated' => 'Escalated', 'Solved' => 'Solved', 'Closed' => 'Closed', 'Reopened' => 'Reopened']), Tables\Filters\SelectFilter::make('type')->options(fn () => TicketType::options(false)), Tables\Filters\SelectFilter::make('priority')->options(['Low' => 'Low', 'Medium' => 'Medium', 'High' => 'High', 'Critical' => 'Critical']), Tables\Filters\TrashedFilter::make(), Tables\Filters\Filter::make('created_date')->form([DatePicker::make('created_from')->label('Dari Tanggal'), DatePicker::make('created_until')->label('Sampai Tanggal')])->query(function (Builder $query, array $data) {
            return $query->when($data['created_from'], fn ($q, $date) => $q->whereDate('created_at', '>=', $date))->when($data['created_until'], fn ($q, $date) => $q->whereDate('created_at', '<=', $date));
        })])->actions(static::getTableActions())->bulkActions([Actions\BulkActionGroup::make([Actions\DeleteBulkAction::make()->visible($isStaff), Actions\ForceDeleteBulkAction::make()->visible($isStaff), Actions\RestoreBulkAction::make()->visible($isStaff)])])->recordUrl(fn ($record) => static::canEdit($record) ? TicketResource::getUrl('edit', [$record]) : TicketResource::getUrl('view', [$record]))->defaultSort('created_at', 'desc');
    }
    ...
    public static function handleAssignAction($record, int $userId): void
    {
        ...
        $targetLayer = TicketLayer::query()->whereIn('role_name', $targetUserRoles, 'and', false)
 ...
 ", 'is_internal' => true, ]);
    ...
    public static function getEscalatableLayers($record): Collection
    {
        return TicketLayer::query()->orderBy('team_key', 'asc')->orderBy('level', 'asc')->get();
    }

    protected static function handleTableEscalate($record, int $targetLayerId, ?string $note): void
    {
        $layer = TicketLayer::query()->find($targetLayerId);
        if (! $layer) {
            return;
        }
 ...
 ", 'is_internal' => true, ]);
        ...
        if ($nextApproval) {
            ...
            Notification::send($notifyUsers, new TicketNotification(ticket: $record, message: " Ticket { $record->ticket_number}
 ...
 ", 'is_internal' => true, ]);
    ...
    {
        return [RelationManagers\CommentsRelationManager::class, RelationManagers\ActivitiesRelationManager::class, RelationManagers\SummonsRelationManager::class];
    }
    ...
    public static function getAssignableUsers(?string $teamKey = null, ?string $assignedGroup = null): array
    {
        ...
        if ($teamKey) {
            $layerRoles = TicketLayer::query()->where('team_key', $teamKey)->pluck('role_name');
            $roleNames = $roleNames->merge($layerRoles);
        } elseif ($assignedGroup) {
            $layerTeamKey = TicketLayer::query()->where('role_name', $assignedGroup)->value('team_key');
            if ($layerTeamKey) {
                $layerRoles = TicketLayer::query()->where('team_key', $layerTeamKey)->pluck('role_name');
                $roleNames = $roleNames->merge($layerRoles);
            }
        }

        $layerMap = TicketLayer::pluck('name', 'role_name');
    ...
    {
        $query = parent::getEloquentQuery()->with(['category', 'ticketApprovals.approver', 'assignee', 'requester', 'attachments', 'helpers']);
        $user = Auth::user();
        if (! $user) {
            return $query;
        }
        ...
        if ($user->isStaff()) {
            $userLayers = TicketLayer::query()->whereIn('role_name', $user->roles->pluck('name'), 'and', false)->get();
